...
class email_template extends CI_Controller {
    ...
    private function loadJs() {
        $this->jsFiles = array();
        $this->jsFiles[] = JS . 'ckeditor/ckeditor.js';
    }
    ...
    function insert_email_template() {
        if ($_POST) {
            ...
            $email_template_detail['email_template_content'] = $this->input->post('email_template_content');
            ...
            if ($this->input->post('email_template_id') != '') {
                ...
                $activity = array();
                $activity['activity_type'] = "add email template";
                $activity['activity_description'] = sprintf(EDIT_EMAIL_TEMPLATE, $this->session->userdata('user_name'));
                $this->user_activity->insert_user_activity($activity);
                $this->lib_message->set_success_message('Template Edited Successfully!!');
            } else {
                ...
                $activity = array();
                $activity['activity_type'] = "edit email template";
                $activity['activity_description'] = sprintf(ADD_EMAIL_TEMPLATE, $this->session->userdata('user_name'));
                $this->user_activity->insert_user_activity($activity);
                $this->lib_message->set_success_message('Template Added Successfully!!');
            }
    ...
    function delete_email_template() {
        if ($_POST) {
            ...
            if ($rows > 0) {
                $activity = array();
                $activity['activity_type'] = "delete email template";
                $activity['activity_description'] = sprintf(DELETE_EMAIL_TEMPLATE, $this->session->userdata('user_name'));
                $this->user_activity->insert_user_activity($activity);
                echo "success";
                exit();
            }
