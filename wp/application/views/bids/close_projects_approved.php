<?php
if ($close_projects_approved && !empty($close_projects_approved)) {
    $CI = &get_instance();
    ?>
    <table id="tbl_close_proj" class="table table-striped table-bordered table-advance table-hover">
        <thead>
            <tr>
                <th>Researcher</th>
                <th>Project Name</th>    
        ...
        <tbody>
            <?php
            foreach ($close_projects_approved as $row) {
                $css_class = '';
                $main_class = '';
                ...
                <tr>
                    ...
                    <td><span id="approve_name_<?php echo $row->project_id; ?>_<?php echo $row->project_country_id; ?>"> <?php if ($row->partner_approved == 0) { ?>-<?php
                            } else {
                                echo $CI->mdl_bids->get_approved_by_name($row->partner_approved_by)->user_name;
                            }
                            ?></span>
                    </td>
                    <td><span id="approve_date_<?php echo $row->project_id; ?>_<?php echo $row->project_country_id; ?>"><?php
                            if ($row->partner_approved == 0) {
                                echo "-";
                            } else {
                                echo date(DATE_DISPLAY_FORMAT, strtotime($row->partner_approved_date));
                            }
                            ?></span>
                    </td>
                    <!--<td id="cp_<?php echo $row->project_id; ?>_<?php echo $row->project_country_id; ?>">
                        <?php
                        if ($row->confirmed_payment == 1) {
                            echo $row->confirmed_payment_amount;
                        } else {
                            ?>
                            <input type="text" placeholder="Received Amount" class="form-control input-small" id="cp_amount_<?php echo $row->project_id; ?>_<?php echo $row->project_country_id; ?>" />
                            <a href="javascript:void(0);" class="btn green confirm_payment" id="confirm_payment_<?php echo $row->project_id; ?>_<?php echo $row->project_country_id; ?>" >I Got Payment</a> 
        <?php } ?>
                    </td>-->
                </tr>
