...
class user_modal extends CI_Model {
    ...
    function check_user_credential($login_user_email, $login_user_password = NULL, $is_check = 0, $forgot_check = 0) {
    ...
    function check_long_key($long_key) {
	...
	function insert_user_company($user_company_data){
		...
		$this->db->query("insert into ".TABLE_PANEL_USER_MASTER." (user_id, user_name, user_email, user_password, user_type, company_id, long_key, is_active,user_entry_date, user_modified_date, user_entry_ip) values(null, '".$user_company_data['user_name']."', '".$user_company_data['user_email']."', '".md5($user_company_data['user_password'])."', '1', '".$company_id."', '".$user_company_data['long_key']."', 0, now(), now(), '".$_SERVER['REMOTE_ADDR']."')");
    ...
    function insert_user($user_data) {
		...
		$this->db->query("insert into " . TABLE_PANEL_USER_MASTER . " (user_id, user_name, user_email, user_password, user_type, company_id, long_key, is_active,user_entry_date, user_modified_date, user_entry_ip) values(null, '" . $user_data['user_name'] . "', '" . $user_data['user_email'] . "', '" . md5($user_data['user_password']) . "', '2', '" . $user_data['company_id'] . "', '" . $user_data['long_key'] . "', '1', now(), now(), '" . $_SERVER['REMOTE_ADDR'] . "')");
    }
